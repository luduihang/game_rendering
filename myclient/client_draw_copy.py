...
class Client:
                    ...
                    if self.mouse_handle.state['quit']:
                        ...
                        if 'game_process' in self.received_data:
                            if self.received_data.get("game_process") == "starting":
                                ...
                                json = self.received_data
                                player_turn = json['whose_turn']
                                spirit = get_voice(json[player_turn]['gender'],json[player_turn]['nickname'])
                                last_char = spirit[-1]  # 取出最后一个字符
                                rest_of_string = spirit[:-1]  # 取出除了最后一个字符之外的所有字符
                                print('last_char',last_char)
                                print('rest_of_string',rest_of_string)
                                self.mouse_handle.draw_pic.music.play_voice(rest_of_string,last_char,json['option'])
    ...
    def run(self):
        try:
            ...
            self.mouse_handle.input_box['password']['text'] = 'ldh'
            self.mouse_handle.input_box['username']['text'] = 'ldh'
            self.mouse_handle.input_box['password']['cursor_position'] = 3
            self.mouse_handle.input_box['username']['cursor_position'] = 3
