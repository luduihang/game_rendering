...
music_button_name = ['stop_to_continue','continue_to_stop','return_play','next_music','shift_list']
class MouseClickHandler:
    def __init__(self,client_socket):
        self.draw_pic = Draw_Pic()
        pygame.mixer.music.set_endevent(pygame.USEREVENT + 1)  # 设置音乐播放结束的事件
        ...
        self.press_return_button = False
        self.have_pressed_button = False

        self.buffer = {}
        self.action_once = True  # 防止操作过于频繁
    ...
    def login_button_pressed_style(self,event):
        if not self.draw_pic.help_signal:
            for button_name, region in Mouce.button_region.items():
                if region.collidepoint(event.pos):
                    ...
                    if button_name == 'help':
                        self.draw_pic.help_signal = True
        if self.draw_pic.help_signal:
            for button_name, region in Mouce.button_help.items():
                if region.collidepoint(event.pos):
                    if button_name == 'help_return':
                        print("help_return button pressed!!!")
                        self.draw_pic.help_signal = False
    ...
    def preparing_button_pressed_style(self, event):
        for button_name, region in Mouce.button_preparing.items():
            if region.collidepoint(event.pos):
                if button_name == 'ready' or button_name == 'cancel_ready':
    ...
    def gaming_button_pressed_style(self,event):
        if self.press_return_button:
            for button_name, region in Mouce.press_return_button.items():
                if region.collidepoint(event.pos):
                    if button_name == "return_preparing":
                        self.press_return_button = False
                        self.have_pressed_button = True
        if not self.press_return_button:
            ...
            if self.draw_pic.target_need_choose:
    ...
    def local_button_pressed_style(self, event):
        for button_name, region in Mouce.button_music.items():
            if region.collidepoint(event.pos):
                if button_name == 'continue_to_stop':
                    if self.action_once:
                        self.draw_pic.music.pause()
                        self.action_once = False
                if button_name == 'return_play':
                    if self.action_once:
                        self.draw_pic.music.restart_song()
                        self.action_once = False
                if button_name == 'next_music':
                    if self.action_once:
                        self.draw_pic.music.next_song()
                        self.action_once = False
                if button_name == 'shift_list':
                    if self.action_once:
                        self.draw_pic.music.next_playlist()
                        self.action_once = False
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if self.state["log_in"] == True:
                    ...
                    self.local_button_pressed_style(event)
    ...
    def run(self,event):
        self.draw_pic.music.play()
        print("123456987")
        while self.running:
            if not self.action_once:
                self.action_once = True
            ...
            if self.state:
                if self.state['log_in'] == True:
                    ...
                    if self.have_pressed_button:
                        self.preparing_json["state"] = False
                        self.key_json = self.buffer
                        self.draw_pic.new_json_signal = True
                        self.update_state("preparing")
                        self.draw_pic.target_need_choose = False
                        self.draw_pic.wait_for_return = False
                        self.have_pressed_button = False
