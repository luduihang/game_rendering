...
from myserver.math import robot
...
class GameRoom:
    def __init__(self):
        ...
        self.robots = {}  # 创建一个字典来存储 robot 实例
        self.robot_id = []
        ...
        self.robot_init()  #加入规定数量的机器人

    def robot_init(self):
        for i in range(self.robot_players):
            username = 'robot'
            username += str(i)
            print('username:', username)
            socket = None
            user_id = str(1000 - i)
            self.robot_id.append(user_id)
            self.robots[user_id] = robot.Robot(user_id)
            self.add_robot(socket,user_id, username, username)

    def add_robot(self,client_socket,user_id,username,password):
        self.archive_manager = UserArchive('folder/user_folder.json')
    ...
    def back_to_ready(self):
        # 重置机器人的参数
        for robot_id in self.robot_id:
            self.robots[robot_id].reset_para()
        ...
        for user_id in self.user_ids_list:
            ...
            self.card_table[user_id] = {"nickname": user_inform['nickname'], "gender": user_inform["gender"],
                                        "preparing": False,
                                        "money": user_inform["money"], "bet_money": 0}
            if 'robot' in self.card_table[user_id]['nickname']:
                self.card_table[user_id]["preparing"] = True
                ...
                if len(self.user_ids_list) != 0 :
                    ...
                    if len(self.clients) == self.max_clients:
                        if all(self.card_table[user_id]["preparing"] for user_id in self.user_ids_list if
                            ...
                            # # 向机器人同步牌桌的下注情况
                            # for robot_id in self.robot_id:
                            #     self.robots[robot_id].update_dick(self.card_table)
                            print('333')
                            break
                else:
                    time.sleep(1)
    ...
    def draw_cards(self):
        ...
        for robot_id in self.robot_id:
            self.robots[robot_id].init_dick(self.user_ids_list,self.card_table[robot_id]['cards'])
        time.sleep(0.1)
    ...
    def place_bet(self):
        ...
        while self.winner < 0:
            ...
            self.timer.start()
            # 如果是机器人做决策的话，就是调用机器人决策函数
            if user_id in self.robot_id:
                self.client_data[user_id] = self.robots[user_id].cal_final_option()
                self.client_received[user_id] = True
                self.client_received_event[user_id].set()
                self.choose_in_time = True
            ...
            if 0 <= self.client_data[user_id]["option"] <= 5:
                ...
                if 0 <= client_option <= 3:
                    ...
                    if self.card_table[str(user_id)]["cards_value"] > self.card_table[str(target_id)]["cards_value"]:
                        ...
                        print('user_lists_copy',self.user_lists_copy)
                        index_value = self.user_lists_copy.index(target_id_str)
                        self.user_lists_copy[index_value] = -1
                        print("挑战成功，被挑战者失败")
