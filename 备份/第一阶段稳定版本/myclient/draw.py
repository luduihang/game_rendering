        ...
        if self.new_json["game_process"] == "battle":
            ...
            if "final_winner" in self.new_json:
                if self.new_json["final_winner"] == int(self.id):
                    arr_icon = ["winner"]
                    public_gaming_pic(arr_icon, self.window_surface)
                    show_font(self.window_surface, '等待自动返回', (0, 0), font_spirit.Destination['HeiTi'], 20, Color.red)
                else:
                    arr_icon = ["game_over"]
                    public_gaming_pic(arr_icon, self.window_surface)
                    show_font(self.window_surface, '等待自动返回', (0, 0), font_spirit.Destination['HeiTi'], 20,
                              Color.red)
        ...
        if self.new_json["game_process"] == "preparing":
            ...
            if str(self.new_json["whose_turn"]) == self.id:
                if not self.target_need_choose:
                    ...
                    show_font(self.window_surface, '请点击头像，选择看牌对象', (330,400), font_spirit.Destination['HeiTi'],45,Color.red)
